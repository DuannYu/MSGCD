...
class NNBatchSampler(Sampler):
    ...
    """
    def __init__(self, model, seen_dataloader, batch_size, train_classes, nn_per_image = 5, using_feat = True, is_norm = True):
        self.batch_size = batch_size
        self.nn_per_image = nn_per_image
        self.using_feat = using_feat
        self.is_norm = is_norm
        # self.num_samples = seen_dataloader.__len__()*batch_size
        # TODO 修改，与 droplast 对应
        self.num_samples = seen_dataloader.dataset.__len__()
        self.mask_lab = None
        self.targets = None
        self.train_classes = train_classes
        self.nn_matrix, self.dist_matrix, self.features = self._build_nn_matrix(model, seen_dataloader)
    ...
    def _predict_batchwise(self, model, seen_dataloader):
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(seen_dataloader):
                ...
                for j in J:
                    A.append(j)
        self.mask_cls = mask_cls.astype(bool)
                        
        model.train()
        model.train(model_is_training) # revert to previous training state

        return torch.stack(A), np.concatenate(mask_labs), np.concatenate(targets)
    
    def _build_nn_matrix(self, model, seen_dataloader):
        # calculate embeddings with model and get targets
        X, self.mask_lab, self.targets = self._predict_batchwise(model, seen_dataloader)
        ...
        return nn_matrix, dist_matrix, X
