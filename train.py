...
from relaxed_loss import *
from sampler import NNBatchSampler

def train(student, train_loader, test_loader, unlabelled_train_loader, dataset, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch > args.start_msgcd:
            balanced_sampler = NNBatchSampler(student, train_loader, args.batch_size, args.train_classes)
            loader = DataLoader(dataset, num_workers = args.num_workers, pin_memory = True, batch_sampler = balanced_sampler, drop_last=False)
        ...
        for batch_idx, batch in enumerate(loader):
            ...
            with torch.cuda.amp.autocast(fp16_scaler is not None):
                ...
                _, pred = student_out.chunk(2)[0].detach().cpu().max(1)
                # teacher_proj, _ = teacher(images); teacher_proj = teacher_proj.detach()
                ...
                rc_loss = rc_loss_f(student_out, student_proj, torch.cat([uq_idxs]*2))
                # rc_loss = rc_loss_f(student_out.chunk(2)[0], student_proj.chunk(2)[1], uq_idxs)
                
                # clustering, unsup
                cluster_loss = cluster_criterion(student_out, teacher_out, epoch)
                cluster_criterion.center_momentum = 0.9
                avg_probs = (student_out / 0.1).softmax(dim=1).mean(dim=0)
                me_max_loss = - torch.sum(torch.log(avg_probs**(-avg_probs))) + math.log(float(len(avg_probs)))
                cluster_loss += args.memax_weight * me_max_loss
                # cluster_loss = 0.1*clu_loss_f((student_out.chunk(2)[0]/0.5).softmax(dim=1), ((student_out.chunk(2)[1]/0.5).softmax(dim=1)))
                ...
                pstr += f'rc_loss: {beta*rc_loss.item():.4f} '
                # pstr += f'nbr_loss: {neighbor_params*neighbor_loss.item():.4f} '
                ...
                if epoch > args.start_msgcd:
                    loss += beta*rc_loss
                    loss += alpha*kmeans_loss
                    pstr += f'kmeans_loss: {alpha*kmeans_loss.item():.4f} '
        ...
        # if epoch % 30 == 0:
        #     torch.save(save_dict, args.model_path+'_{}.pt'.format(str(epoch)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--start_msgcd', default=60, type=int)
    ...
    import random
    torch.backends.cudnn.deterministic=True; np.random.seed(0); random.seed(0) 
    ...
    # TRAIN
    # ----------------------
    train(model, train_loader, test_loader_labelled, test_loader_unlabelled, train_dataset, args)
